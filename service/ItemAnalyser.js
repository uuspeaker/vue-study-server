...
class ItemAnalyser{
    ...
    extractValidItem(){
      ...
      for (var i = 0; i < length; i++) {
        ...
        //log.debug("开始解析",item)
        //去掉题目编号大于总长度的
        if(item.itemNo > length || item.itemNo == 0) {
    ...
    analyseItemPolygon(){
      ...
      var itemPolygonLast = {
        itemNo: this.validItems[length-1].itemNo,
        leftX: this.validItems[length-1]['Polygon'][0].X,
        leftY: this.validItems[length-1]['Polygon'][0].Y,
        rightX: this.validItems[length-1]['Polygon'][0].X + this.itemMaxLength,
        rightY: this.maxY
      }
      itemPolygons.push(itemPolygonLast)
